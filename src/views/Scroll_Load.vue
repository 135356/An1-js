  ...
  <div>
    ...
    <div :style="'height:'+scroll_state.top+'px;'">
      <div v-if="!scroll_state.end&&scroll_state.load_show" class="load" :style="'height:'+scroll_state.default_top+'px;'">
        ...
        <img src="../assets/img/load/scroll_load.gif" height="60" width="60"/></div>
      <div v-if="scroll_state.end" @click="backTop()" class="load_end" :style="'height:'+scroll_state.default_top+'px;'">
        <div class="turn_text">点击回到顶部...</div>
      </div>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    destroyed () {
      window.removeEventListener('scroll', this.throttle(document.documentElement, this.funF));
    }
  }
</script>
<style scoped>
